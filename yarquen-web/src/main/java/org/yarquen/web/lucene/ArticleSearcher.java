...
public class ArticleSearcher {
	...
	private void activeSelectedFacetsOnTree(FacetTree skillTree,
			List<Skill> skills) {
		for (Skill skill : skills) {
			skillTree.activeBranch(skill.getCategoryBranch());
		}
	}
	...
	private void addToFacetTree(FacetResultNode facetResultNode,
			FacetTreeNode treeNode) {
		final String[] components = facetResultNode.getLabel().components;
		final String lastComponent = components[components.length - 1];
		// TODO: improve this
		boolean leaf = lastComponent.equals("0") || lastComponent.equals("1")
				|| lastComponent.equals("2") || lastComponent.equals("3");
		...
		LOGGER.trace("adding branch to tree: {}", branch.getCode());
		treeNode.addBranch(branch, facetResultNode.getValue());
		...
		if (numSubResults > 0) {
			for (FacetResultNode subResult : facetResultNode.getSubResults()) {
				addToFacetTree(subResult, treeNode);
			}
		}
	}
	...
	private void populateFacets(YarquenFacets facetsCount,
		...
		// provided skills
		facetsCount.setProvidedSkillTree(new FacetTree());
		...
		for (FacetResultNode facetResultNode : providedSkillFacetsResults) {
			...
			addToFacetTree(facetResultNode, facetsCount.getProvidedSkillTree());
		}

		// required skills
		facetsCount.setRequiredSkillTree(new FacetTree());
		...
		for (FacetResultNode facetResultNode : requiredSkillFacetsResults) {
			LOGGER.trace("node: {}", facetResultNode);
			addToFacetTree(facetResultNode, facetsCount.getRequiredSkillTree());
		}
		...
		if (results > lastIndex) {
			for (int i = lastIndex; i < results; i++) {
				...
				if (skillFacetName.equals(Article.Facets.PROVIDED_SKILL
						.toString())) {
					addToFacetTree(cfc, facetsCount.getProvidedSkillTree());
				} else if (skillFacetName.equals(Article.Facets.REQUIRED_SKILL
						.toString())) {
					addToFacetTree(cfc, facetsCount.getRequiredSkillTree());
				} else {
		...
		// generate complete codes
		facetsCount.getProvidedSkillTree().genereteCompleteCodes();
		facetsCount.getRequiredSkillTree().genereteCompleteCodes();
	}
}
